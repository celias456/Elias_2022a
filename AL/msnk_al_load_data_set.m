function [data,Sigma_hat,c,first_observation,Sigma_u_sd,theta,prior_information] = msnk_al_load_data_set(data_set_identifier)
...
if data_set_identifier == 1 %pre-great moderation
    ...
    Sigma_hat = load('msnk_al_sigma_hat_pre_great_moderation.csv'); 
    ...
    parameter_info = load('msnk_al_parameters_pre_great_moderation.csv');
...
elseif data_set_identifier == 2 %great moderation
    ...
    %Covariance matrix used for the variance of the proposal distribution in the MH algorithm (obtained from Dynare)
    Sigma_hat = load('msnk_al_sigma_hat_great_moderation.csv'); 
    Sigma_hat = nearestSPD(Sigma_hat); %The "nearestSPD" function ensures that the matrix P will be symmetric and postive semi-definite so that it can be used in the "mvnrnd" function
    
    %Load the parameter information
    parameter_info = load('msnk_al_parameters_great_moderation.csv');
    prior_value = parameter_info(:,1);
    prior_dist = parameter_info(:,2);
    hyp_1 = parameter_info(:,3);
    hyp_2 = parameter_info(:,4);
...
elseif data_set_identifier == 3  %pre-great recession
    ...
    %Covariance matrix used for the variance of the proposal distribution in the MH algorithm (obtained from Dynare)
    Sigma_hat = load('msnk_al_sigma_hat_pre_great_recession.csv'); 
    Sigma_hat = nearestSPD(Sigma_hat); %The "nearestSPD" function ensures that the matrix P will be symmetric and postive semi-definite so that it can be used in the "mvnrnd" function
    
    %Load the parameter information
    parameter_info = load('msnk_al_parameters_pre_great_recession.csv');
    prior_value = parameter_info(:,1);
    prior_dist = parameter_info(:,2);
    hyp_1 = parameter_info(:,3);
    hyp_2 = parameter_info(:,4);
    ...
    %Covariance matrix used for the variance of the proposal distribution in the MH algorithm (obtained from Dynare)
    Sigma_hat = load('msnk_al_sigma_hat_full_data_set.csv'); 
    Sigma_hat = nearestSPD(Sigma_hat); %The "nearestSPD" function ensures that the matrix P will be symmetric and postive semi-definite so that it can be used in the "mvnrnd" function
    
    %Load the parameter information
    parameter_info = load('msnk_al_parameters_full_data_set.csv');
    prior_value = parameter_info(:,1);
    prior_dist = parameter_info(:,2);
    hyp_1 = parameter_info(:,3);
    hyp_2 = parameter_info(:,4);
